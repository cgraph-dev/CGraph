...
**Status:** ✅ Complete
**Commit:** `cf9e1b29` — `feat(10-03): message extras polish — save buttons, mobile disappearing, timer icons`
**Files changed:** 13 (1 created, 12 modified) — 499 insertions
...
**Conversation integration** (`use-conversation-header.tsx`, `use-conversation-setup.ts`, `conversation-screen.tsx`):
...
**Mobile** (`message-bubble.tsx`): Added `Ionicons timer-outline` icon (12px) next to timestamp when `item.expires_at` is present, with appropriate opacity for own vs. other messages
...
- Timer indicator added to mobile only (web already had timestamp area; `messageId` prop added for save button instead)
