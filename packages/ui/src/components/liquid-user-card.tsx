...
/** User avatar with image or gradient-initials fallback and optional status dot. */
export function LiquidAvatar({ src, alt, fallback, size = 'md', status }: LiquidAvatarProps) {
  return (
...
export interface LiquidUserCardProps
  ...
  status?: LiquidAvatarProps['status'];
  /** Trailing action slot. */
  action?: ReactNode;
  children?: ReactNode;
}

export const LiquidUserCard = forwardRef<HTMLDivElement, LiquidUserCardProps>(
  (
    { className, size, interactive, avatarSrc, name, subtitle, status, action, children, ...props },
    ref
  ) => {
...
LiquidUserCard.displayName = 'LiquidUserCard';
