...
| Secret            | Required | Description                                  |
| ----------------- | -------- | -------------------------------------------- |
| `DISCORD_WEBHOOK` | No       | CGraph webhook URL for release notifications |
...
| ------------------------------- | ------- | ---------------------------------------------------- |
| `DISCORD_NOTIFICATIONS_ENABLED` | `false` | Set to `true` to enable CGraph release notifications |
...
### 4. CGraph Webhook (Optional)

1. In your CGraph server, go to Server Settings → Integrations → Webhooks
