...
All MyBB features have been added to the TypeScript type system in `/apps/web/src/stores/forumStore.ts`.
...
<PollWidget
  poll={thread.poll}
  threadId={thread.id}
  isCreator={user.id === thread.authorId}
/>
```
...
<motion.div
  className="glass-fluid card-seamless smooth-hover p-4 rounded-xl"
