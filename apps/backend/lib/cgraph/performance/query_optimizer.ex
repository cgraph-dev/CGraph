defmodule CGraph.Performance.QueryOptimizer do
  ...
  def cursor_paginate(query, opts \\ []) do
    ...
    query = query
      |> apply_cursor_filter(after_cursor, before_cursor, order_field, order_dir)
    ...
    end_cursor = build_end_cursor(results, order_field)

    {:ok, results, %{end_cursor: end_cursor, has_more: has_more, limit: limit}}
  end

  defp apply_cursor_filter(query, nil, nil, _order_field, _order_dir), do: query
  defp apply_cursor_filter(query, after_cursor, _before, order_field, order_dir) when not is_nil(after_cursor) do
    apply_after_cursor(query, after_cursor, order_field, order_dir)
  end
  defp apply_cursor_filter(query, _after, before_cursor, order_field, order_dir) when not is_nil(before_cursor) do
    apply_before_cursor(query, before_cursor, order_field, order_dir)
  end

  defp apply_after_cursor(query, cursor, order_field, order_dir) do
    case decode_cursor(cursor) do
      {:ok, value} -> apply_cursor_where(query, value, order_field, order_dir, :after)
      :error -> query
    end
  end

  defp apply_before_cursor(query, cursor, order_field, order_dir) do
    case decode_cursor(cursor) do
      {:ok, value} -> apply_cursor_where(query, value, order_field, order_dir, :before)
      :error -> query
    end
  end

  defp apply_cursor_where(query, value, order_field, :desc, :after) do
    where(query, [r], field(r, ^order_field) < ^value)
  end
  defp apply_cursor_where(query, value, order_field, :asc, :after) do
    where(query, [r], field(r, ^order_field) > ^value)
  end
  defp apply_cursor_where(query, value, order_field, :desc, :before) do
    where(query, [r], field(r, ^order_field) > ^value)
  end
  defp apply_cursor_where(query, value, order_field, :asc, :before) do
    where(query, [r], field(r, ^order_field) < ^value)
  end

  defp build_end_cursor([], _order_field), do: nil
  defp build_end_cursor(results, order_field) do
    results |> List.last() |> Map.get(order_field) |> encode_cursor()
  end
  ...
  defp decode_cursor(cursor) do
    with {:ok, value} <- Base.url_decode64(cursor, padding: false) do
