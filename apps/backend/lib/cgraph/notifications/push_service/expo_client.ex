...
defmodule Cgraph.Notifications.PushService.WebPushClient do
  ...
  defp do_send(endpoint, auth_keys, payload, opts, retry_count) when retry_count < @max_retries do
    ...
    with :ok <- validate_payload_size(payload_bytes),
         {:ok, encrypted, salt, server_public_key} <- encrypt_for_push(auth_keys, payload_bytes) do
      headers = build_request_headers(endpoint, config, opts, salt, server_public_key)
      handle_push_response(http_post(endpoint, headers, encrypted), endpoint, auth_keys, payload, opts, retry_count)
    end
  end
  ...
  defp validate_payload_size(payload_bytes) do
  ...
  defp encrypt_for_push(auth_keys, payload_bytes) do
    ...
    case encrypt_payload(payload_bytes, p256dh, auth) do
      {:ok, _encrypted, _salt, _server_public_key} = result -> result
      {:error, reason} ->
  ...
  defp build_request_headers(endpoint, config, opts, salt, server_public_key) do
    ttl = Keyword.get(opts, :ttl, 86_400)
    urgency = Keyword.get(opts, :urgency, "normal")
    topic = Keyword.get(opts, :topic)
    build_vapid_headers(endpoint, config, ttl, urgency, topic, salt, server_public_key)
  end

  defp handle_push_response({:ok, status, _headers, _body}, _endpoint, _auth_keys, _payload, _opts, _retry_count)
       when status in [200, 201], do: {:ok, :sent}

  defp handle_push_response({:ok, 410, _headers, _body}, _endpoint, _auth_keys, _payload, _opts, _retry_count),
    do: {:error, :gone}

  defp handle_push_response({:ok, 413, _headers, _body}, _endpoint, _auth_keys, _payload, _opts, _retry_count),
    do: {:error, :payload_too_large}

  defp handle_push_response({:ok, 429, _headers, _body}, endpoint, auth_keys, payload, opts, retry_count) do
    Process.sleep(1000 * (retry_count + 1))
    do_send(endpoint, auth_keys, payload, opts, retry_count + 1)
  end

  defp handle_push_response({:ok, status, _headers, body}, _endpoint, _auth_keys, _payload, _opts, _retry_count) do
    Logger.warning("Web push failed (#{status}): #{body}")
    {:error, :send_failed}
  end

  defp handle_push_response({:error, reason}, endpoint, auth_keys, payload, opts, retry_count) do
