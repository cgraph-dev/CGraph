defmodule CgraphWeb.PresenceChannel do
  ...
  def terminate(_reason, socket) do
    ...
    if user && socket.assigns[:tracked] do
      # Record last seen before full disconnect
      Presence.record_last_seen(user.id)
      
      # Small delay before marking truly offline (handles reconnects)
      spawn(fn ->
        Process.sleep(@offline_grace_period_ms)
        
        # Check if user reconnected during grace period
        unless Presence.user_online?(user.id) do
          Phoenix.PubSub.broadcast(
            Cgraph.PubSub,
            "users:online",
            {:user_offline, user.id, DateTime.utc_now()}
          )
        end
      end)
    end
