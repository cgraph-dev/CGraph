...
| 7.4 | voice_room_name produces vc_ prefixed name | ✅ |
...
**LiveKit controller `authorize_room_access` bug**: `CGraph.Groups.get_member(group_id, user.id)` passed a string `group_id` instead of a group struct. Fixed to use `get_group(group_id)` → `get_member_by_user(group, user.id)` chain.
