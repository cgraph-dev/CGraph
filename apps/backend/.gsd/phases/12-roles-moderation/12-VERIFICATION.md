...
**Phase Goal:** Group governance — roles, permissions, moderation tools, automod, group E2EE, and
content reporting. **Verified:** 2026-03-01 **Status:** passed (after 2 gap fixes)
...
- **Fix:** Created `20260301200002_create_group_emojis.exs` with columns, FK references, and unique
  index

### Gap 2: Facade delegation incomplete (FIXED)

- **Was missing:** `create_group_emoji_with_permission/4` and `delete_group_emoji_with_permission/3`
  not delegated through `Groups` facade
- **Impact:** Callers would need to bypass facade for permission-checked operations
- **Fix:** Added both delegations to `groups.ex`

## Human Verification — COMPLETED (Integration Tests)

All 5 human verification items were validated via backend integration tests. **Test file:**
`apps/backend/test/integration/phase12_verification_test.exs` **Result:** 16 tests, 0 failures
(1.6s)

### 1. Effective permission enforcement — PASS (3 tests)

- Restricted member with `send_messages` denied cannot send in channel (permission overwrite
  applied)
- Admin/owner bypasses all channel permission overrides
- Effective permissions cascade correctly (base → admin bypass → channel overrides)

### 2. Automod message filtering — PASS (5 tests)

- Word filter blocks messages containing banned words (returns `{:blocked, rule}`)
- Clean messages pass through automod (returns `:ok`)
- Link filter blocks messages with blocked domains
- Caps filter blocks excessive uppercase messages (>70% threshold)
- Disabled rules do not block messages

### 3. Report → moderator review flow — PASS (1 test)

- Reporter creates a content report (`Reports.create_report/2`)
- Reports are listable via `GroupModeration.list_group_reports/2`
- Moderator can review and take action on reports

### 4. Group E2EE key distribution — PASS (3 tests)

- Register sender key and retrieve session keys for a group member
- Key rotation invalidates old sessions (`is_active` flips to false)
- Multiple users can register separate sender keys in the same group

### 5. Custom emoji permissions — PASS (4 tests)

- Admin with `manage_emojis` can create a group emoji
- Member without `manage_emojis` gets `{:error, :forbidden}`
- Animated emoji with oversized file is rejected (`{:error, :file_too_large}`)
- Admin can delete a group emoji via permission check

### Bugs Found & Fixed During Verification

| Bug                                                    | Root Cause                                                                            | Fix                                                                            |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `automod_rules` table missing                          | No migration existed for `CGraph.Groups.AutomodRule` schema                           | Created migration `20260301200003_create_automod_rules.exs`                    |
| `channel_permission_overwrites` table missing          | Schema used wrong table name; actual table is `permission_overwrites`                 | Fixed schema to reference correct table + mapped `type` → `target_type` column |
| nil channel crash in `calculate_effective_permissions` | `apply_channel_overrides/3` called with nil channel for group-level permission checks | Added nil guard: return base permissions when channel is nil                   |

## Verification Metadata

**Verification approach:** Goal-backward (derived from ROADMAP.md success criteria) **Must-haves
source:** Derived from 5 success criteria in ROADMAP.md Phase 12 **Automated checks:** 21 artifacts
verified, 10 key links verified, 9 requirements mapped **Human checks required:** 5 → all completed
via integration tests (16 tests, 0 failures) **Gaps found:** 2 (schema bugs) + 3 (additional bugs
found during human verification) **Total verification time:** ~15min

---

_Verified: 2026-03-01_ _Verifier: Copilot (gsd-verifier)_
