...
- `useMarketplaceFacade().balance` now returns `useGamificationStore.getState().coins` (real balance)
...
- `useVoiceRecording()` now delegates to real `useVoiceRecorder` from voice-message-recorder component
...
- Fixed path from `@/components/conversation/voice-message-recorder` to `@/components/voice-message-recorder`
