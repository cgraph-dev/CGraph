defmodule Cgraph.UploadsSecurityTest do
  ...
  describe "image optimization" do
    ...
    @tag :imagemagick
    test "optimize_image generates thumbnails when ImageMagick available" do
      # Create a small test image (1x1 red pixel JPEG)
      jpeg_data = create_test_jpeg()

      # Write to temp file
      test_dir = Path.join(System.tmp_dir!(), "cgraph_test_#{:rand.uniform(1_000_000)}")
      File.mkdir_p!(test_dir)
      source_path = Path.join(test_dir, "test_image.jpg")
      File.write!(source_path, jpeg_data)
      ...
      assert is_binary(original_url)

      # If thumbnails were created, verify URLs
      if thumb_url do
        assert String.contains?(thumb_url, "test_id_thumb")
      end

      if preview_url do
        assert String.contains?(preview_url, "test_id_preview")
      end
  ...
  # Helper to create a minimal valid JPEG for testing
