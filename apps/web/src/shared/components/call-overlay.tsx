...
export function CallOverlay({
  ...
  if (pip) {
    return (
      <motion.div
        drag
        dragMomentum={false}
        className="fixed bottom-24 right-6 z-[9999] h-[180px] w-[320px] cursor-move overflow-hidden rounded-xl border border-white/10 bg-[#111214] shadow-2xl"
        initial={{ scale: 0.8, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.8, opacity: 0 }}
      >
        {/* Mini participant grid */}
        <div className="grid h-full w-full grid-cols-2 gap-px bg-black/50">
        ...
        <div className="absolute right-2 top-2 flex gap-1">
          <button
            onClick={togglePip}
            className="rounded bg-black/60 p-1 text-white/60 transition-colors hover:text-white"
          >
            <ArrowsPointingOutIcon className="h-3.5 w-3.5" />
          </button>
          <button
            onClick={onEndCall}
            className="rounded bg-red-600 p-1 text-white transition-colors hover:bg-red-500"
          >
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[9999] flex flex-col bg-[#111214]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      >
        ...
        <div className="flex flex-1 gap-3 overflow-hidden px-4 pb-2">
          ...
          {hasScreenShare && screensharer && (
            <div className="relative flex flex-1 items-center justify-center overflow-hidden rounded-xl border border-white/[0.06] bg-black/40">
              <ComputerDesktopIcon className="h-16 w-16 text-white/10" />
              <div className="absolute bottom-3 left-3 flex items-center gap-2 rounded-lg bg-black/60 px-3 py-1.5">
...
function ParticipantTile({
  ...
  return (
    <div
      className={cn(
        'relative flex h-full items-center justify-center overflow-hidden rounded-xl bg-[#1e1f22]',
    ...
    >
      ...
      >
        <span
          className={cn('truncate text-white/80', compact ? 'text-[10px]' : 'text-sm font-medium')}
        >
          {p.name}
          {p.isSelf && ' (You)'}
        </span>
        {p.isMuted && (
          <MicrophoneIcon
            className={cn('shrink-0 text-red-400', compact ? 'h-2.5 w-2.5' : 'h-3.5 w-3.5')}
          />
        )}
        {p.isCameraOff && (
          <VideoCameraSlashIcon
            className={cn('shrink-0 text-white/30', compact ? 'h-2.5 w-2.5' : 'h-3.5 w-3.5')}
          />
        )}
      </div>
    </div>
