...
const TABS: { id: QuestTab; name: string; icon: React.ReactNode; color: string }[] = [
  { id: 'active', name: 'Active', icon: <ClipboardDocumentListIcon className="h-4 w-4" />, color: 'from-purple-500 to-pink-500' },
  { id: 'daily', name: 'Daily', icon: <FireIcon className="h-4 w-4" />, color: 'from-orange-500 to-red-500' },
  { id: 'weekly', name: 'Weekly', icon: <CalendarIcon className="h-4 w-4" />, color: 'from-blue-500 to-cyan-500' },
  { id: 'completed', name: 'Completed', icon: <CheckCircleIcon className="h-4 w-4" />, color: 'from-green-500 to-emerald-500' },
];
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                quest.type === 'daily' ? 'bg-orange-500/20 text-orange-400' :
                quest.type === 'weekly' ? 'bg-blue-500/20 text-blue-400' :
            ...
            <h3 className="text-lg font-semibold text-white mt-2">{quest.title}</h3>
          ...
          <div className="flex items-center gap-2">
            {quest.xpReward > 0 && (
              <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-purple-500/20">
        ...
        <div className="flex items-center justify-end gap-3">
          {onAccept && (
            <motion.button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-primary-500 to-purple-500 text-white font-medium text-sm disabled:opacity-50"
            >
              {isAccepting ? (
                <motion.div
          ...
          {canClaim && onClaim && (
            <motion.button
              ...
              disabled={isClaiming}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 text-white font-medium text-sm disabled:opacity-50"
            >
              {isClaiming ? (
                <motion.div
                  className="h-4 w-4 border-2 border-white border-t-transparent rounded-full"
...
function ObjectiveItem({ objective }: { objective: QuestObjective }) {
  ...
  return (
    <div className="relative">
      ...
      <div className="ml-6 h-1 bg-dark-700 rounded-full overflow-hidden">
        <motion.div
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <motion.button
              ...
              className="p-2 rounded-xl bg-dark-800 text-gray-400 hover:text-white transition-colors"
            >
        ...
        {isLoading ? (
          ...
          <GlassCard className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-400 mb-2">
              {selectedTab === 'active' ? 'No active quests' :
               selectedTab === 'completed' ? 'No completed quests yet' :
               'No available quests'}
            </h3>
            ...
            <AnimatePresence mode="popLayout">
              {displayedQuests.map(quest => (
                <QuestCard
                  key={quest.id}
                  quest={quest}
                  onAccept={selectedTab !== 'active' && selectedTab !== 'completed' 
                    ? () => handleAcceptQuest(quest.id) 
                    : undefined}
                  onClaim={selectedTab === 'active' && quest.completed 
