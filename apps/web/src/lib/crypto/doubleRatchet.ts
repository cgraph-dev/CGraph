...
async function generateDHKeyPair(): Promise<KeyPair> {
  const keyPair = await crypto.subtle.generateKey(
    { name: 'ECDH', namedCurve: 'P-384' }, // P-384 for enhanced security
    true,
    ['deriveBits']
  );
...
export class DoubleRatchetEngine {
  ...
  async encryptMessage(
    ...
    const fullAD = associatedData
      ? this.concatArrays(headerBytes, associatedData)
      : headerBytes;
  ...
  async decryptMessage(message: EncryptedMessage): Promise<DecryptedMessage> {
    ...
    const needsRatchet = !this.state.DHr || 
      !this.arraysEqual(header.dh, this.state.DHr);
  ...
  private pruneSkippedKeys(): void {
    if (this.state.MKSKIPPED.size > this.config.maxSkippedMessages) {
      const keysToDelete = Array.from(this.state.MKSKIPPED.keys())
        .slice(0, this.state.MKSKIPPED.size - this.config.maxSkippedMessages);
  ...
  private serializeHeader(header: MessageHeader): Uint8Array {
    ...
    view.setUint32(offset, header.pn, false); offset += 4;
    view.setUint32(offset, header.n, false); offset += 4;
    view.setBigUint64(offset, BigInt(header.timestamp), false); offset += 8;
    buffer[offset++] = sessionIdBytes.length;
    buffer.set(sessionIdBytes, offset); offset += sessionIdBytes.length;
  ...
  async importState(stateJson: string): Promise<void> {
    ...
    this.state = {
      ...
      Nr: imported.Nr,
      PN: imported.PN,
      MKSKIPPED: new Map(imported.MKSKIPPED.map(([k, v]: [string, number[]]) => [k, new Uint8Array(v)])),
