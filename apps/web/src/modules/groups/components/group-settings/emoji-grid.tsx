...
export interface GroupEmoji {
  ...
  uploadedBy: string;
  createdAt: string;
  /** When 'lottie', render via LottieRenderer instead of <img>. */
  animationFormat?: 'gif' | 'apng' | 'webp' | 'lottie' | null;
  /** CDN URL to the Lottie JSON file. */
  lottieUrl?: string;
}
    ...
    <GlassCard variant="frosted" className="p-6">
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="mt-1 text-sm text-gray-600">Upload PNG, GIF, WebP, or Lottie JSON files</p>
          <motion.button
            ...
            >
              ...
              {emoji.isAnimated && (
                <span className="absolute right-1 top-1 rounded bg-purple-500/20 px-1 text-[10px] text-purple-400">
                  {emoji.animationFormat === 'lottie' ? 'Lottie' : 'GIF'}
                </span>
              )}
