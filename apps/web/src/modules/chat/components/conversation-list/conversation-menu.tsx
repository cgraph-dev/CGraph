      ...
      <AnimatePresence>
        {showDropdown && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute right-0 top-full z-50 mt-1 w-40 rounded-xl border border-white/[0.08] bg-white/[0.04] py-1 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => onAction('pin')}
              className="flex w-full items-center gap-2 px-3 py-2 text-sm text-gray-300 hover:bg-white/[0.08]"
            >
