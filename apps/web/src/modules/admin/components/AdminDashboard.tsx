      ...
      >
        ...
        <div className="border-b border-white/10 p-6">
          ...
          >
            ...
            {!sidebarCollapsed && <span className="text-lg font-bold">Admin Panel</span>}
          </motion.div>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-lg px-4 py-3 transition-all ${
                activeTab === item.id
                  ? 'border border-purple-500/30 bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-white'
        ...
        <button
          ...
          className="border-t border-white/10 p-4 text-gray-500 transition-colors hover:text-white"
        >
...
function ModerationQueueItem({ item }: { item: ModerationItem }) {
  ...
  return (
    <div className="flex cursor-pointer items-center gap-4 rounded-lg bg-black/20 p-3 transition-colors hover:bg-white/5">
      <span className={`rounded px-2 py-1 text-xs font-medium ${riskColors[item.riskLevel]}`}>
    ...
    >
      ...
      <div className="space-y-4">
        {filteredEvents.length === 0 ? (
          <div className="py-12 text-center text-gray-500">No events found for this filter</div>
        ) : (
          filteredEvents.map((event) => (
            <div
              key={event.id}
              className="flex cursor-pointer items-center gap-4 rounded-xl border border-white/10 bg-white/5 p-4 transition-colors hover:border-purple-500/30"
            >
              <div className="flex h-16 w-16 items-center justify-center rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 text-2xl">
                🎉
              </div>
              <div className="flex-1">
              ...
              <button className="rounded-lg bg-white/10 px-4 py-2 text-sm transition-colors hover:bg-white/20">
                Manage
              </button>
            </div>
          ))
