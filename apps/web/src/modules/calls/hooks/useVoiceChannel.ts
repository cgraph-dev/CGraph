...
export function useVoiceChannel(): UseVoiceChannelReturn {
  ...
  const joinChannel = useCallback(async (channelId: string, groupId: string) => {
      ...
      const joinResult = await new Promise<{ token: string; room_name: string }>(
        (resolve, reject) => {
          channel
            ...
            .receive('error', (resp) => reject(new Error((resp as { reason?: string }).reason ?? 'join_failed')))
  ...
  }, [currentChannelId, socketManager, setConnected, setConnecting, setError, addChannelMember, removeChannelMember, updateMemberState, setChannelMembers]);
