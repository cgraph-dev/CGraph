...
const WarningCard = memo(function WarningCard({ warning, onRevoke }: WarningCardProps) {
  ...
  return (
    <div
      className={`rounded-lg border p-3 ${warning.isActive ? 'border-red-800/40 bg-red-900/10' : 'bg-white/[0.04]/30 border-white/[0.08]'}`}
    >
  ...
  return (
    <div className="space-y-4">
      ...
      {currentUserWarnings.length === 0 && (
        <div className="bg-white/[0.04]/30 rounded-lg border border-white/[0.08] py-6 text-center text-gray-500">
          No warnings issued
        </div>
      )}
    </div>
