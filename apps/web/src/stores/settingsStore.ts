...
function mapSettingsFromApi(data: ApiSettings): UserSettings {
  return {
    ...
    keyboard: {
      keyboardShortcutsEnabled: data.keyboard_shortcuts_enabled ?? DEFAULT_KEYBOARD_SETTINGS.keyboardShortcutsEnabled,
...
function mapSettingsToApi(settings: Partial<UserSettings>): Record<string, unknown> {
  ...
  if (settings.keyboard) {
    ...
    if (k.keyboardShortcutsEnabled !== undefined) result.keyboard_shortcuts_enabled = k.keyboardShortcutsEnabled;
  ...
  persist(
    (set, get) => ({
      ...
      updateKeyboardSettings: async (keyboardSettings) => {
        ...
        try {
          await api.put('/api/v1/settings', mapSettingsToApi({ 
            keyboard: { ...previousSettings.keyboard, ...keyboardSettings } 
          }));
          set({ isSaving: false, lastSyncedAt: Date.now() });
        } catch (error) {
...
export { DEFAULT_SETTINGS, DEFAULT_NOTIFICATION_SETTINGS, DEFAULT_PRIVACY_SETTINGS, DEFAULT_APPEARANCE_SETTINGS };
