...
import { LockClosedIcon, SparklesIcon, CrownIcon } from '@heroicons/react/24/outline';
...
export function PremiumThemeGate({
  ...
  const hasAccess = tierHierarchy[userTier as keyof typeof tierHierarchy] >= tierHierarchy[requiredTier];
    ...
    <div className={`relative ${className}`}>
      ...
      {showPreview && (
        <div className="relative overflow-hidden rounded-xl">
          <div className="blur-sm brightness-75 pointer-events-none select-none">
            {children}
          </div>
          ...
          >
            <GlassCard variant="frosted" glow className="p-6 max-w-sm text-center">
              {/* Lock icon with animation */}
              <motion.div
                className="mx-auto w-16 h-16 rounded-full bg-dark-800 flex items-center justify-center mb-4"
                animate={{
                  boxShadow: [
              ...
              <motion.button
                ...
                className={`w-full py-3 px-6 rounded-xl font-semibold text-white ${config.bgColor} shadow-lg hover:shadow-xl transition-shadow`}
              >
      ...
      {!showPreview && (
        <motion.div
          ...
          className="p-4 rounded-xl border-2 border-dashed border-gray-700 bg-dark-800/50"
        >
          <div className="flex items-center gap-3">
            ...
            <motion.button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium text-white ${config.bgColor}`}
            >
              Upgrade
            </motion.button>
          </div>
                ...
                >
                  {plan.highlighted && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                      <span className={`px-3 py-1 text-xs font-bold text-white rounded-full ${config.bgColor}`}>
                        Recommended
                      </span>
                    </div>
                  )}
