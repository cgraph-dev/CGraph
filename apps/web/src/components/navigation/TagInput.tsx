...
export default function TagInput({
  ...
  const filteredSuggestions = suggestions.filter(
    (s) =>
      s.label.toLowerCase().includes(inputValue.toLowerCase()) && !tags.some((t) => t.id === s.id)
  );
    ...
    <div className={`relative ${className}`}>
      <div className="flex flex-wrap gap-2 rounded-lg border border-dark-600 bg-dark-700 p-2 focus-within:border-transparent focus-within:ring-2 focus-within:ring-primary-500">
        {tags.map((tag) => (
          <span
            key={tag.id}
            className="flex items-center gap-1 rounded-md bg-primary-600/20 px-2 py-1 text-sm text-primary-400"
          >
            {tag.label}
            <button
        ...
        <input
          ...
          className="min-w-[100px] flex-1 bg-transparent text-sm text-white placeholder-gray-500 outline-none"
