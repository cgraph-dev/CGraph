...
const PERMISSIONS = {
  ADMINISTRATOR: { value: 1 << 0, label: 'Administrator', description: 'Full access to all settings', danger: true },
  MANAGE_GROUP: { value: 1 << 1, label: 'Manage Group', description: 'Edit group settings and info' },
  MANAGE_ROLES: { value: 1 << 2, label: 'Manage Roles', description: 'Create and edit roles below this one' },
  MANAGE_CHANNELS: { value: 1 << 3, label: 'Manage Channels', description: 'Create, edit, and delete channels' },
  KICK_MEMBERS: { value: 1 << 4, label: 'Kick Members', description: 'Remove members from the group' },
  BAN_MEMBERS: { value: 1 << 5, label: 'Ban Members', description: 'Permanently ban members' },
  MANAGE_MESSAGES: { value: 1 << 6, label: 'Manage Messages', description: 'Delete and pin messages' },
  MENTION_EVERYONE: { value: 1 << 7, label: 'Mention Everyone', description: 'Use @everyone and @here' },
  MANAGE_NICKNAMES: { value: 1 << 8, label: 'Manage Nicknames', description: 'Change other members\' nicknames' },
  MANAGE_EMOJIS: { value: 1 << 9, label: 'Manage Emojis', description: 'Add and remove custom emojis' },
  VIEW_AUDIT_LOG: { value: 1 << 10, label: 'View Audit Log', description: 'Access the group audit log' },
  PRIORITY_SPEAKER: { value: 1 << 11, label: 'Priority Speaker', description: 'Be heard over others in voice' },
  STREAM: { value: 1 << 12, label: 'Stream', description: 'Go live in voice channels' },
  SEND_MESSAGES: { value: 1 << 13, label: 'Send Messages', description: 'Send messages in text channels' },
  ...
  ADD_REACTIONS: { value: 1 << 16, label: 'Add Reactions', description: 'Add reactions to messages' },
  ...
  DEAFEN_MEMBERS: { value: 1 << 20, label: 'Deafen Members', description: 'Deafen others in voice' },
  MOVE_MEMBERS: { value: 1 << 21, label: 'Move Members', description: 'Move members between voice channels' },
};

const ROLE_COLORS = [
  '#10b981', '#8b5cf6', '#ec4899', '#f59e0b', '#ef4444',
  '#06b6d4', '#84cc16', '#f97316', '#6366f1', '#14b8a6',
  '#e11d48', '#7c3aed', '#0ea5e9', '#22c55e', '#eab308',
];
...
export function RoleManager({ groupId, className = '' }: RoleManagerProps) {
  ...
  const handleUpdateRole = useCallback((updates: Partial<Role>) => {
      ...
      <div className="w-64 border-r border-gray-700/50 p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <motion.button
            ...
            className="p-1.5 rounded-lg bg-primary-600/20 text-primary-400 hover:bg-primary-600/30"
          >
          ...
          {roles.map((role) => (
            ...
            >
              ...
              >
                ...
                />
                <span className="flex-1 text-left text-sm font-medium text-gray-300 truncate">
                  {role.name}
                </span>
                {role.isDefault && (
        ...
        {selectedRole ? (
          ...
          <div className="h-full flex items-center justify-center">
            <div className="text-center">
              <ShieldCheckIcon className="h-16 w-16 text-gray-700 mx-auto mb-4" />
...
function RoleEditor({
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['general', 'permissions']));
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onSave}
            className="px-4 py-2 rounded-lg bg-primary-600 text-white font-semibold flex items-center gap-2"
          >
                    ...
                    {ROLE_COLORS.map((color) => (
                      <motion.button
                        ...
                        className={`w-8 h-8 rounded-full ${
                          role.color === color ? 'ring-2 ring-white ring-offset-2 ring-offset-dark-800' : ''
                        }`}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 rounded-lg bg-dark-800">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">Show members with this role in a separate group</p>
                    </div>
                    <Toggle
                ...
                {Object.entries(PERMISSIONS).map(([key, perm]) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-lg ${
                      perm.danger ? 'bg-red-500/5 border border-red-500/20' : 'bg-dark-800'
                    }`}
                  >
                    <div>
                      <span className={`font-medium ${perm.danger ? 'text-red-400' : 'text-white'}`}>
                        {perm.label}
                      </span>
