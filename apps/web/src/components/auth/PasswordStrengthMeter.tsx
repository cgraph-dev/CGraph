...
export const PasswordStrengthMeter: React.FC<PasswordStrengthMeterProps> = ({
  ...
  const requirements = useMemo(() => {
    return DEFAULT_REQUIREMENTS.map((r) => r.label === 'At least 8 characters'
      ? { ...r, label: `At least ${minLength} characters`, test: (p: string) => p.length >= minLength }
      : r
    );
  }, [minLength]);
        ...
        <div className="flex gap-1">
          {STRENGTH_LEVELS.map((level, index) => (
            <motion.div
              ...
              className={`
                flex-1 h-1.5 rounded-full origin-left
                ${passedRequirements > index ? level.color : 'bg-white/10'}
              `}
            />
          ))}
        </div>
            ...
            {requirements.map((requirement, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-xs ${passed ? 'text-green-400' : 'text-white/50'}`}
                  >
                    {requirement.label}
                  </span>
                </motion.li>
