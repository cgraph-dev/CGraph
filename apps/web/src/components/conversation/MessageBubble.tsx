...
/**
 * MessageBubble - Renders a single chat message with all features
 * Extracted from Conversation.tsx for better maintainability
 */
      ...
      >
        ...
        <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} max-w-[70%]`}>
          ...
          {isOwn && message.metadata?.readBy && Array.isArray(message.metadata.readBy) && (
            <ReadReceipts readBy={message.metadata.readBy as ReadByUser[]} />
          )}
        </div>
      </div>
    );
...
function ReadReceipts({ readBy }: { readBy: ReadByUser[] }) {
  return (
    <motion.div
    ...
    <div className="relative flex items-center gap-1">
      ...
      {isMenuOpen && (
        <div className="absolute right-0 top-full z-50 mt-1 w-32 rounded-lg bg-dark-800 py-1 shadow-lg ring-1 ring-white/10">
          <ActionMenuItem icon={<EditIcon />} label="Edit" onClick={onEdit} />
          <ActionMenuItem icon={<PinIcon />} label="Pin" onClick={onPin} />
          <ActionMenuItem icon={<ForwardIcon />} label="Forward" onClick={onForward} />
          <ActionMenuItem icon={<DeleteIcon />} label="Delete" onClick={onDelete} danger />
        </div>
      )}
    </div>
  );
...
function ActionMenuItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-dark-700 ${
