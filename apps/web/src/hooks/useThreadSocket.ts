...
export function useThreadSocket(
  ...
  const vote = useCallback(async (value: 1 | -1 | 0): Promise<ThreadVotePayload> => {
  ...
  const voteComment = useCallback(async (commentId: string, value: 1 | -1 | 0): Promise<CommentVotePayload> => {
    if (!threadId) throw new Error('No thread ID');
    return socketManager.voteOnComment(threadId, commentId, value);
  }, [threadId]);

  const postComment = useCallback(async (content: string, parentId?: string): Promise<{ comment_id: string }> => {
    if (!threadId) throw new Error('No thread ID');
  ...
  const votePoll = useCallback(async (optionId: string): Promise<{ poll: ThreadPollData }> => {
    if (!threadId) throw new Error('No thread ID');
