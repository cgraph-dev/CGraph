...
const TelegramAttachmentPicker = memo(
  ({ visible, onClose, onSelectAssets, maxSelection = 10 }: TelegramAttachmentPickerProps) => {
    ...
    const [_contactSearchQuery, setContactSearchQuery] = useState('');
    ...
    const renderCameraTile = () => (
      <TouchableOpacity style={styles.cameraTile} onPress={openCamera} activeOpacity={0.8}>
    ...
    const filteredContacts = contacts.filter(
      (contact) =>
        ...
        contact.phoneNumbers?.some((p: Contacts.PhoneNumber) =>
          p.number?.includes(contactSearchQuery)
        )
    );
        ...
        <View style={styles.container}>
          ...
          >
            ...
            <View style={styles.header}>
              ...
              {selectedAssets.size > 0 && (
                <TouchableOpacity style={styles.sendButton} onPress={handleSendSelection}>
                  <Text style={styles.sendButtonText}>{selectedAssets.size}</Text>
            ...
            {isLoading ? (
              ...
              <View style={styles.fallbackContainer}>
                ...
                <Text style={[styles.fallbackTitle, { color: colors.text }]}>Open Gallery</Text>
                ...
                >
                  ...
                  <Text style={[styles.fallbackCameraButtonText, { color: colors.primary }]}>
                    Take Photo/Video
                  </Text>
                </TouchableOpacity>
              </View>
          ...
          {showAlbumPicker && (
            <View style={styles.albumPickerOverlay}>
              ...
              <View
                style={[
                  styles.albumPickerContainer,
                  { backgroundColor: isDark ? '#1c1c1e' : '#fff' },
                ]}
              >
                <View style={styles.albumPickerHeader}>
                  <Text style={[styles.albumPickerTitle, { color: colors.text }]}>
                    Select Album
                  </Text>
                  <TouchableOpacity
                    onPress={() => setShowAlbumPicker(false)}
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    ...
                    >
                      ...
                      {((item === null && selectedAlbum === null) ||
                        item?.id === selectedAlbum?.id) && (
