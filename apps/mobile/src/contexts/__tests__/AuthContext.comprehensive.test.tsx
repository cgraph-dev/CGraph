...
describe('AuthContext', () => {
  ...
  describe('Token Management', () => {
    it('should store tokens securely on login', async () => {
      ...
      expect(mockSecureStore.setItemAsync).toHaveBeenCalledWith('access_token', tokens.accessToken);
  ...
  describe('Wallet Authentication', () => {
    it('should generate wallet signature challenge', () => {
      ...
      const challenge = generateChallenge('0x1234567890abcdef', Date.now());
...
describe('Password Reset Flow', () => {
  ...
  it('should validate reset token format', () => {
    const isValidResetToken = (token: string) => /^[a-zA-Z0-9]{64}$/.test(token);
