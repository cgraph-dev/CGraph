...
interface UseGroupsReturn extends GroupsState {
  ...
  // Member functions
  loadMembers: (groupId: string, options?: { role?: GroupRole; status?: 'online' | 'offline' | 'all' }) => Promise<void>;
  ...
  banMember: (groupId: string, userId: string, options?: { reason?: string; duration?: number }) => Promise<boolean>;
  
  // Channel functions
  loadChannels: (groupId: string) => Promise<void>;
  createChannel: (groupId: string, data: { name: string; type: 'text' | 'voice' | 'announcement' | 'forum'; categoryId?: string }) => Promise<GroupChannel | null>;
  
  // Invite functions
  loadInvites: (groupId: string) => Promise<void>;
  createInvite: (groupId: string, options?: { maxUses?: number; expiresIn?: number }) => Promise<GroupInvite | null>;
...
export function useGroups(options: UseGroupsOptions = {}): UseGroupsReturn {
  ...
  const getPublicGroups = useCallback(async (options?: { category?: string; search?: string }): Promise<Group[]> => {
    try {
  ...
  const createGroup = useCallback(async (data: CreateGroupRequest): Promise<Group | null> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to create group' }));
      return null;
    }
  }, []);

  const updateGroup = useCallback(async (groupId: string, data: UpdateGroupRequest): Promise<Group | null> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to update group' }));
      return null;
    }
  }, []);
  ...
  const deleteGroup = useCallback(async (groupId: string): Promise<boolean> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to delete group' }));
      return false;
    }
  }, []);
  ...
  const joinGroup = useCallback(async (groupId: string): Promise<boolean> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to join group' }));
      return false;
    }
  }, []);
  ...
  const leaveGroup = useCallback(async (groupId: string): Promise<boolean> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to leave group' }));
      return false;
    }
  }, []);
  ...
  const loadMembers = useCallback(async (groupId: string, options?: { role?: GroupRole; status?: 'online' | 'offline' | 'all' }) => {
  ...
  const updateMemberRole = useCallback(async (groupId: string, userId: string, role: GroupRole): Promise<boolean> => {
    try {
  ...
  const kickMember = useCallback(async (groupId: string, userId: string, reason?: string): Promise<boolean> => {
    try {
  ...
  const banMember = useCallback(async (groupId: string, userId: string, options?: { reason?: string; duration?: number }): Promise<boolean> => {
    try {
