...
interface UseCalendarReturn extends CalendarState {
  ...
  // Reminders
  addReminder: (eventId: string, type: 'notification' | 'email', minutes: number) => Promise<Reminder | null>;
...
export function useCalendar(options: UseCalendarOptions = {}): UseCalendarReturn {
  ...
  const loadUpcoming = useCallback(async (limit = 10) => {
  ...
  const createEvent = useCallback(async (data: CreateEventRequest): Promise<CalendarEvent | null> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to create event' }));
      return null;
    }
  }, []);

  const updateEvent = useCallback(async (eventId: string, data: UpdateEventRequest): Promise<CalendarEvent | null> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to update event' }));
      return null;
    }
  }, []);

  const deleteEvent = useCallback(async (eventId: string, deleteFuture = false): Promise<boolean> => {
    try {
      ...
      setState(prev => ({ ...prev, error: error.message || 'Failed to delete event' }));
      return false;
    }
  }, []);
  ...
  const rsvp = useCallback(async (eventId: string, status: 'going' | 'maybe' | 'not_going'): Promise<boolean> => {
    try {
  ...
  const inviteUsers = useCallback(async (eventId: string, userIds: string[]): Promise<boolean> => {
    try {
  ...
  const addReminder = useCallback(async (
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        selectedEvent: prev.selectedEvent?.id === eventId
  ...
  const removeReminder = useCallback(async (eventId: string, reminderId: string): Promise<boolean> => {
    try {
  ...
  const eventsForDate = useCallback((date: string): CalendarEvent[] => {
