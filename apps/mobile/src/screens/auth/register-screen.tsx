...
export default function RegisterScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    const animations = fadeAnims.map((anim, index) =>
      Animated.parallel([
        Animated.timing(anim, { toValue: 1, duration: durations.smooth.ms, delay: index * 80, useNativeDriver: true }),
        Animated.timing(translateYAnims[index], { toValue: 0, duration: durations.smooth.ms, delay: index * 80, useNativeDriver: true }),
      ])
    );
    Animated.stagger(80, animations).start();
    Animated.loop(
      Animated.sequence([
        Animated.timing(glowOpacity, { toValue: 0.6, duration: durations.loop.ms, useNativeDriver: true }),
        Animated.timing(glowOpacity, { toValue: 0.3, duration: durations.loop.ms, useNativeDriver: true }),
      ])
    ).start();
  }, []);
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
            <View style={styles.content}>
              ...
              <LinearGradient
                ...
                start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.formContainer}>
                <View style={styles.form}>
                  <RegisterFormFields
                    email={reg.email} setEmail={reg.setEmail}
                    username={reg.username} setUsername={reg.setUsername}
                    password={reg.password} setPassword={reg.setPassword}
                    confirmPassword={reg.confirmPassword} setConfirmPassword={reg.setConfirmPassword}
                    agreedToTerms={reg.agreedToTerms} setAgreedToTerms={reg.setAgreedToTerms}
                    focusedField={reg.focusedField} setFocusedField={reg.setFocusedField}
                    fadeAnims={fadeAnims} translateYAnims={translateYAnims}
                  />
                  ...
                  {/* OAuth */}
                  <Animated.View style={{ opacity: fadeAnims[7], transform: [{ translateY: translateYAnims[7] }] }}>
                    <AuthDivider text="Or continue with" />
                    <OAuthButtonGroup variant="icon" providers={['google', 'apple', 'facebook', 'tiktok']}
                      onSuccess={() => {}} onError={(error) => { if (!error.message.includes('cancelled')) console.error('OAuth error:', error); }} />
                  </Animated.View>
                </View>
              </LinearGradient>

              {/* Footer */}
              <Animated.View style={[styles.footer, { opacity: fadeAnims[7], transform: [{ translateY: translateYAnims[7] }] }]}>
