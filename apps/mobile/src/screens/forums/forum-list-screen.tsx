...
export default function ForumListScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    fetchForums();
    Animated.parallel([
      Animated.timing(headerOpacity, { toValue: 1, duration: durations.slower.ms, useNativeDriver: true }),
      Animated.spring(headerTranslateY, { toValue: 0, friction: 8, tension: 40, useNativeDriver: true }),
    ]).start();
    setTimeout(() => {
      Animated.spring(fabScale, { toValue: 1, friction: 6, tension: 40, useNativeDriver: true }).start();
    }, 600);
  }, []);
    ...
    <Animated.View style={[styles.headerContainer, { opacity: headerOpacity, transform: [{ translateY: headerTranslateY }] }]}>
      <GlassCard variant="frosted" intensity="subtle" style={styles.searchCard}>
        <TouchableOpacity style={styles.searchBar}
          ...
          <Text style={[styles.searchPlaceholder, { color: colors.textSecondary }]}>Search communities...</Text>
        </TouchableOpacity>
      </GlassCard>
      <View style={styles.categoriesRow}>
        {['Popular', 'New', 'Growing'].map((category, index) => (
          <TouchableOpacity key={category} onPress={() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)}>
            {index === 0 ? (
              <LinearGradient colors={['#3b82f6', '#8b5cf6']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.categoryChipActive}>
  ...
  if (isLoading) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.skeletonContainer}>{[1, 2, 3, 4, 5].map((i) => <ForumCardSkeleton key={i} />)}</View>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <FlatList
        data={forums} renderItem={renderForum} keyExtractor={(item) => item.id}
        ListHeaderComponent={renderHeader}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary} colors={['#3b82f6', '#8b5cf6']} />}
      ...
      <Animated.View style={[styles.fabContainer, { transform: [{ scale: fabScale }] }]}>
        <TouchableOpacity activeOpacity={0.8}
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); navigation.navigate('CreateForum'); }}>
          <LinearGradient colors={['#3b82f6', '#8b5cf6']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.fab}>
