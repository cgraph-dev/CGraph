...
export function SearchForm({ onSuccess }: SearchFormProps) {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.timing(cardOpacity, { toValue: 1, duration: durations.dramatic.ms, useNativeDriver: true }),
    ]).start();
  }, []);
  ...
  useEffect(() => {
    if (success) {
      ...
      Animated.parallel([
        Animated.spring(successScale, { toValue: 1, tension: 100, friction: 6, useNativeDriver: true }),
        Animated.timing(successOpacity, { toValue: 1, duration: durations.slow.ms, useNativeDriver: true }),
      ]).start();
    } else {
  ...
  useEffect(() => {
    if (error) {
      ...
      Animated.sequence([
        Animated.timing(errorShake, { toValue: 10, duration: durations.stagger.ms, useNativeDriver: true }),
        Animated.timing(errorShake, { toValue: -10, duration: durations.stagger.ms, useNativeDriver: true }),
        Animated.timing(errorShake, { toValue: 10, duration: durations.stagger.ms, useNativeDriver: true }),
        Animated.timing(errorShake, { toValue: 0, duration: durations.stagger.ms, useNativeDriver: true }),
      ]).start();
    }
  }, [error]);
  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      let errorMessage = typeof apiErr.response?.data?.error === 'object'
    ...
    <Animated.View style={{ opacity: cardOpacity, transform: [{ scale: cardScale }] }}>
      <GlassCard variant="neon" intensity="medium" style={styles.mainCard}>
        <View style={styles.iconContainer}>
          <LinearGradient colors={['#8B5CF6', '#06B6D4']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.iconGradient}>
        ...
        {/* Animated Input */}
        <Animated.View style={[styles.inputWrapper, { shadowColor: '#8B5CF6', shadowOpacity: inputGlow, shadowRadius: 12, transform: [{ translateX: errorShake }] }]}>
          <Animated.View style={[styles.inputGlow, { backgroundColor: glowColor }]} />
          <View style={styles.inputContainer}>
            <Ionicons name="search" size={20} color={isFocused ? '#8B5CF6' : '#6B7280'} style={styles.inputIcon} />
            <TextInput
              style={styles.input}
              placeholder="Username, email, or #ID"
              placeholderTextColor="#6B7280"
              value={searchInput}
              onChangeText={(text) => { setSearchInput(text); setError(''); setSuccess(false); }}
            ...
            {searchInput.length > 0 && (
              <TouchableOpacity onPress={() => { Haptics.selectionAsync(); setSearchInput(''); }}>
        ...
        {success && (
          <Animated.View style={[styles.successContainer, { opacity: successOpacity, transform: [{ scale: successScale }] }]}>
            <LinearGradient colors={['rgba(34, 197, 94, 0.2)', 'rgba(16, 185, 129, 0.2)']} style={styles.successGradient}>
        ...
        <Animated.View style={{ transform: [{ scale: buttonScale }] }}>
          <TouchableOpacity onPressIn={handlePressIn} onPressOut={handlePressOut} onPress={handleSendRequest} activeOpacity={0.9} disabled={loading || !searchInput.trim()}>
            <LinearGradient
