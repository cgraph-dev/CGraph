...
- `ci.yml`: lint (ESLint), format (Prettier), typecheck (TS), tests (Vitest/Jest), backend
  `mix test`.
...
- Registry credentials, database URL (for integration), TURN credentials (if e2e tests), SENTRY_DSN
  (optional).
