...
- Events: `signal:offer`, `signal:answer`, `signal:ice_candidate`, `participant:joined`, `participant:left`, `participant:media_updated`, `call:ended`.
- REST helpers: `POST /api/v1/calls` (create room), `POST /api/v1/calls/{room_id}/join`, `POST /api/v1/calls/{room_id}/leave`.
...
## Incomplete / TODO
- Voice channels (persistent Discord-style) marked planned.
