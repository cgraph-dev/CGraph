...
CGraph uses Phoenix Presence, a distributed CRDT-based presence tracking system that provides
real-time online/offline status across the cluster. For large channels (1000+ members), we've added
a sampled presence layer that reduces bandwidth and memory usage while maintaining user experience.

This document details the architectural decisions, patterns, and best practices for working with
presence tracking.
...
- **Sampled presence** for large-scale channels (v0.7.32+)
...
Standard Phoenix Presence works beautifully for conversations and small groups. But broadcasting
presence updates for 100,000+ member channels would saturate both server bandwidth and client
memory. We solved this with tiered sampling.

### The Problem at Scale

Imagine a public channel with 500,000 members. If 50,000 of them are online, standard presence
would:
...
For huge channels, we stop showing individual online users entirely and just show an approximate
count. That's what CGraph does, and it works.

### HyperLogLog for Counting

We use the HyperLogLog algorithm to count unique online users. It's probabilistic but incredibly
space-efficient:
...
When we sample 1% of online users for display, we don't pick randomly each time. That would make the
"who's online" list flicker constantly. Instead, we hash each user ID and use the hash to determine
if they're in the sample. The same users always appear in the sample, so the list stays stable.
...
**Problem:** Component remounting causes rapid channel join/leave cycles, flooding the server with
presence diffs.
...
**Problem:** Channels can be in various states (joining, joined, closed, errored, leaving) and
attempting operations on channels in bad states causes errors.
...
**Problem:** Not cleaning up subscriptions and channels leads to memory leaks, especially in
...
**Problem:** Async operations (socket connection, channel join) complete out of order, causing stale
state updates.
...
**Cause:** Attempting to join channel before socket connection established **Solution:** Always
await `socketManager.connect()` before joining

#### "Channel already joined"

**Cause:** Multiple join attempts without checking existing channel **Solution:** Implemented in
v0.7.16 - socket manager now handles this automatically

#### "User constantly appearing/disappearing"

**Cause:** Join/leave loop (see section above) **Solution:** Applied debouncing + component tracking
fixes

#### "Presence state out of sync"

**Cause:** Network interruption or backend restart **Solution:** Phoenix Presence automatically
