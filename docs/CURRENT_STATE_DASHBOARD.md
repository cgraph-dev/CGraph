...
> **Version: 0.9.33** | Generated: February 20, 2026
...
> **⚠️ Scores re-calibrated February 20, 2026 after deep codebase audit (Session 32).** Previous
> scores were inflated. See [V1_ACTION_PLAN.md](V1_ACTION_PLAN.md) for gap analysis.
...
| **Architecture**  | OK     | 7/10   | ~~8 dead packages~~ **deleted**; mobile triple-directory overlap (screens/features/modules not consolidated); ~~2,261 lines dead backend code~~ **reduced to ~400 lines** (3 GenServers wired, 2 ConnectionPool modules deleted); Wave 0 structural migrations partially complete; React 19 APIs adopted (forwardRef removed, useOptimistic in use); SoftDelete fully adopted (22 files); cursor pagination migrated (8 forum files) |
| **Tests**         | OK     | 7/10   | 1,908 backend tests (~82%); web 4,968 tests (205 files); mobile 27 test files; 14 E2E flows; CI coverage gates enforce 60% web / 75% backend; **load tests never run**; coverage % unverified without execution                                                                                                                                                                                                                      |
| **Security**      | OK     | 8/10   | Real X3DH ECDH + PQXDH E2EE; 3-layer rate limiting; Guardian JWT + token blacklist (640-line 3-tier); 7 CI security tools; CSP hardened; **device binding claimed but NOT implemented**; mobile E2EE skips DH4; external pen test overdue                                                                                                                                                                                            |
| **Documentation** | OK     | 6/10   | ~~PROJECT_STATUS.md contradictions~~ **fixed**; ~~3 different composite scores~~ **unified at 7.0/10 then 7.6**; stale tier names cleaned; dead code documented; device binding claim removed; phase statuses honest; CLAUDE.md updated with audit findings                                                                                                                                                                          |
| **Observability** | OK     | 7/10   | **Grafana Cloud deployed** (cgraphdev.grafana.net) — Alloy v1.13.1 sidecar on Fly.io scrapes `/metrics` → Grafana Cloud Prometheus; 64+ metrics (Phoenix/Ecto/Oban/WebSocket/RateLimiter/Security/VM); dashboard (10 panels) + 6 alert rules imported and active; SLO GenServer + Service Registry wired; OTel SDK present; Sentry active                                                                                            |
...
| **CI/CD**         | OK     | 8/10   | 16 GH Actions; coverage hard-fail; 7 security scan tools; **bundle size check is now hard-fail**; Lighthouse audits; Renovate auto-updates; staging deploy; load test + chaos test workflows exist but untested                                                                                                                                                                                                                      |

**Composite Score: 7.8/10** — Solid engineering foundation with real crypto, testing, and CI. Key
gaps: load tests never run, device binding unimplemented, mobile directory overlap
(screens/features/modules), external pen test overdue.
...
| ------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0.9.33  | 2026-02-20 | **Grafana Cloud observability deployed**: Alloy v1.13.1 metrics sidecar on Fly.io (scrapes `/metrics` → Grafana Cloud Prometheus remote write); 18 new telemetry metrics (WebSocket, RateLimiter, Security); SoftDelete `not_deleted` converted to defmacro + `exclude_deleted` pipe fn; session_signing_salt compile/runtime conflict fixed; Resend email + Stripe webhook configured; Dockerfile `libc6-compat` for Alloy glibc binary; production deploy healthy (2 processes: app with Alloy sidecar, pgbouncer) |
...
<sub>**CGraph Dashboard** • Version 0.9.33 • Updated: February 20, 2026</sub>
