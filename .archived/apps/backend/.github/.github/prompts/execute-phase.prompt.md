...
tools:
  ['readFile', 'editFiles', 'listDirectory', 'textSearch', 'runInTerminal', 'runSubagent', 'todos']
---
...
<execution_context> ../instructions/ui-brand.instructions.md ../skills/execute-phase/SKILL.md
</execution_context>
...
- `--gaps-only` — Execute only gap closure plans (plans with `gap_closure: true` in frontmatter).
  Use after verify-work creates fix plans.

@.gsd/ROADMAP.md @.gsd/STATE.md </context>
...
7. **Verify phase goal** Check config:
   `WORKFLOW_VERIFIER=$(cat .gsd/config.json 2>/dev/null | grep -o '"verifier"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false' || echo "true")`
...
10. **Commit phase completion** Check `COMMIT_PLANNING_DOCS` from config.json (default: true). If
    false: Skip git operations for .gsd/ files. If true: Bundle all phase metadata updates in one
    commit:
...
Score: {N}/{M} must-haves verified Report: .gsd/phases/{phase_dir}/{phase}-VERIFICATION.md
...
Spawn all plans in a wave with a single message containing multiple Task calls, with inlined
content:
...
<checkpoint_handling> Plans with `autonomous: false` have checkpoints. The execute-phase.md workflow
handles the full checkpoint flow:
