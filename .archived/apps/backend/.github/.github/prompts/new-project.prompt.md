...
tools: ['readFile', 'runInTerminal', 'editFiles', 'createFile', 'createDirectory', 'runSubagent']
---

<objective>

Initialize a new project through unified flow: questioning → research (optional) → requirements →
roadmap.

This is the most leveraged moment in any project. Deep questioning here means better plans, better
execution, better outcomes. One command takes you from idea to ready-for-planning.
...
../instructions/questioning.instructions.md ../instructions/ui-brand.instructions.md
@.gsd/templates/project.md @.gsd/templates/requirements.md
...
Based on what they said, ask follow-up questions that dig into their response. Use HumanAgent MCP
(HumanAgent_Chat) with options that probe what they mentioned — interpretations, clarifications,
concrete examples.
...
As you go, mentally check the context checklist from `questioning.md`. If gaps remain, weave
questions naturally. Don't suddenly switch to checklist mode.
...
These spawn additional agents during planning/execution. They add tokens and time but improve
quality.
...
- options:
  - "Research first (Recommended)" — Discover standard stacks, expected features, architecture
    patterns
...
**Atomic commits:** Each phase commits its artifacts immediately. If context is lost, artifacts
persist.
