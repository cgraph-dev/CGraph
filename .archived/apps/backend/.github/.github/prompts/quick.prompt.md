...
name: 'gsd:quick'
description:
  'Execute a quick task with GSD guarantees (atomic commits, state tracking) but skip optional
  agents'
...
<execution_context> Orchestration is inline - no separate workflow file. Quick mode is deliberately
simpler than full GSD. </execution_context>
...
Note: For quick tasks producing multiple plans (rare), spawn executors in parallel waves per
