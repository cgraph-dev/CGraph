...
Each mapper agent explores a focus area and **writes documents directly** to `.gsd/codebase/`. The
orchestrator only receives confirmations, keeping context usage minimal.

Output: .gsd/codebase/ folder with 7 structured documents about the codebase state. </objective>

<execution_context> ../skills/map-codebase/SKILL.md </execution_context>

<context>
Focus area: $ARGUMENTS (optional - if provided, tells agents to focus on specific subsystem)

**Load project state if exists:** Check for .gsd/STATE.md - loads context if project already
initialized
