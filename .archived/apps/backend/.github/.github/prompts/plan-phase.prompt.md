...
agent: '📋 GSD Planner'
tools:
  ['readFile', 'editFiles', 'runInTerminal', 'listDirectory', 'textSearch', 'runSubagent', 'fetch']
---

<execution_context> ../instructions/ui-brand.instructions.md </execution_context>
...
**Orchestrator role:** Parse arguments, validate phase, research domain (unless skipped or exists),
spawn gsd-planner agent, verify plans with gsd-plan-checker, iterate until plans pass or max
iterations reached, present results.

**Why subagents:** Research and planning burn context fast. Verification uses fresh context. User
sees the flow between agents in main context. </objective>
...
**If not found:** Error with available phases. **If found:** Extract phase number, name,
description.
...
<phase_context> **IMPORTANT:** If CONTEXT.md exists below, it contains user decisions from
/discuss-phase.md.
...
**Prior decisions from STATE.md:** {decisions} </additional_context>
...
**If exists:** Offer: 1) Continue planning (add more plans), 2) View existing, 3) Replan from
scratch. Wait for response.

## 7. Read Context Files

Read and store context file contents for the planner agent. The `@` syntax does not work across
...
<downstream_consumer> Output consumed by /execute-phase.md Plans must be executable prompts with:
...
- Check config:
  `WORKFLOW_PLAN_CHECK=$(cat .gsd/config.json 2>/dev/null | grep -o '"plan_check"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false' || echo "true")`
...
IMPORTANT: If phase context exists below, it contains USER DECISIONS from /discuss-phase.md. Plans
MUST honor these decisions. Flag as issue if plans contradict user's stated vision.
...
Research: {Completed | Used existing | Skipped} Verification: {Passed | Passed with override |
Skipped}
