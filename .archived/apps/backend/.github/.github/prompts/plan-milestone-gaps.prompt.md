...
Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and
offers to plan each phase.

One command creates all fix phases — no manual `/add-phase.md` per gap. </objective>
...
**Original intent (for prioritization):** @.gsd/PROJECT.md @.gsd/REQUIREMENTS.md

**Current state:** @.gsd/ROADMAP.md @.gsd/STATE.md </context>
...
**Goal:** {derived from gaps being closed} **Requirements:** {REQ-IDs being satisfied} **Gap
Closure:** Closes gaps from audit
...
**Phases added:** {N} - {M} **Gaps addressed:** {count} requirements, {count} integration, {count}
flows
...
`/audit-milestone.md` — re-audit to verify gaps closed `/complete-milestone.md {version}` — archive
when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/plan-phase.md` next </success_criteria>
