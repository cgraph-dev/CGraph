...
**Note:** This is a standalone research command. For most workflows, use `/plan-phase.md` which
...
**Orchestrator role:** Parse phase, validate against roadmap, check existing research, gather
context, spawn researcher agent, present results.

**Why subagent:** Research burns context fast (Web Search MCP, Context7 queries, source
verification). Fresh 200k context for investigation. Main context stays lean for user interaction.
</objective>
...
```markdown
<research_type> Phase Research — investigating HOW to implement a specific phase well.
</research_type>
...
<downstream_consumer> Your RESEARCH.md will be loaded by `/plan-phase.md` which uses specific
sections:
...
**`## RESEARCH INCONCLUSIVE`:** Show what was attempted, offer: Add context, Try different mode,
Manual.
...
<prior_state> Research file: @.gsd/phases/${PHASE}-{slug}/${PHASE}-RESEARCH.md </prior_state>
