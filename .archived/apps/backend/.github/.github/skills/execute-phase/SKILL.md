...
name: execute-phase
description:
  Execute all plans in a phase using wave-based parallel execution. Orchestrator delegates plan
  execution to subagents, manages waves and checkpoints.
---
...
<core_principle> The orchestrator's job is coordination, not execution. Each subagent loads the full
execute-plan context itself. Orchestrator discovers plans, analyzes dependencies, groups into waves,
spawns agents, handles checkpoints, collects results. </core_principle>

<required_reading> Read STATE.md before any operation to load project context. Read config.json for
planning behavior settings. </required_reading>
...
Store `BRANCHING_STRATEGY` and templates for use in branch creation step. </step>
...
**Note:** All subsequent plan commits go to this branch. User handles merging based on their
workflow. </step>
...
If all plans filtered out, report "No matching incomplete plans" and exit. </step>
...
1. **Describe what's being built (BEFORE spawning):**
   ...
   **Examples:**
   ...
   - Good: "Procedural terrain generator using Perlin noise — creates height maps, biome zones, and
     collision meshes. Required before vehicle physics can interact with ground."
...
4. **Report completion and what was built:**
   ...
   **Examples:**
   ...
   - Good: "Terrain system complete — 3 biome types, height-based texturing, physics collision
     meshes. Vehicle physics (Wave 3) can now reference ground surfaces."
...
**Why fresh agent instead of resume:** Resume relies on Copilot Code's internal serialization which
breaks with parallel tool calls. Fresh agents with explicit state are more reliable and maintain
full context.
...
**Score:** {N}/{M} must-haves verified **Report:** {phase_dir}/{phase}-VERIFICATION.md
...
<context_efficiency> Orchestrator: ~10-15% context (frontmatter, spawning, results). Subagents:
Fresh 200k each (full workflow + execution). No polling (Task blocks). No context bleed.
</context_efficiency>
