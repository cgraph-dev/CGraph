...
- `CreatorRoutes` — Stripe Connect onboarding, paid forums, subscriptions, earnings, payouts,
  analytics
...
- `MarketplaceController` — admin marketplace management (+
  `marketplace_controller/settings_actions.ex`)
...
- **Web3 wallet** — Ethereum wallet auth via full SIWE (EIP-4361 Sign-In with Ethereum) flow:
  challenge nonce → SIWE-formatted message → EIP-191 personal sign → signature recovery (ex_keccak +
  ex_secp256k1). Includes domain binding, nonce expiration (300s), and `parse_siwe_message/1` for
...
- `paid_forum_subscriptions` — Paid forum subscription records (Stripe subscription + billing
  period)
...
- **Stripe Connect (Express):** Creator monetization — creators onboard via Express accounts, funds
  flow directly to connected accounts with `application_fee_percent` (default 15%)
- **Paid forum subscriptions:** Recurring Stripe Subscriptions with `transfer_data.destination` to
  route payments to creators, platform fee auto-deducted
