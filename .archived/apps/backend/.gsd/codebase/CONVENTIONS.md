...
>
> **Missing facades**: Two major contexts have **no facade file at all**: `notifications` and
> `subscriptions`. Their modules are accessed directly via sub-module paths (e.g.,
> `CGraph.Notifications.Delivery`, `CGraph.Subscriptions.TierLimits`).

**Facade + Delegation Pattern** (introduced by `Creators` context):

Larger contexts use `defdelegate` in the facade module to keep the public API clean while splitting
implementation across specialized sub-modules:
...
Sub-module naming convention: named by **responsibility** — `ConnectOnboarding`, `PaidSubscription`,
...
**Style B — `action_fallback` + `render_data` helper** (used by `PaymentController`,
`CoinShopController`):
...
- User access: `conn.assigns.current_user` (pipeline-assigned) or
  `Guardian.Plug.current_resource(conn)`
- Multi-clause error matching with descriptive atom errors (`:below_minimum`, `:already_subscribed`,
  etc.)
...
Route modules are then imported into the main `Router` via `import` + macro call. API routes follow
...
- **PubSub subscriptions**: user-specific + global topics subscribed in
...
- **Pattern-matched handlers**: one `handle_info` clause per PubSub event atom (`:xp_awarded`,
  `:achievement_unlocked`, `:battle_pass_purchased`, etc.)
...
- **Security**: nonce deleted after successful verification (replay protection), domain allowlist
  (`@allowed_domains`), expiration validation
