...
**Effective permission system merges role bitmasks with per-channel allow/deny overrides, enforced
in group_channel.ex for join, send, delete, and pin operations.**
...
- Built `calculate_effective_permissions/3` in roles.ex: base OR → admin bypass → role overrides →
  member overrides
