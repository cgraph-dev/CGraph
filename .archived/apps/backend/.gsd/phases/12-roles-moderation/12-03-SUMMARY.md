...
**Automod enforcement pipeline checks every non-admin group message against configurable rules
(word/link/spam/caps filters) with 4 action types, plus mobile settings UI for rule management.**
...
- `execute_action/4` dispatches: `:delete` (block), `:warn` (allow + push warning), `:mute` (block +
  TODO mute), `:flag_for_review` (allow + async report)
...
- Modal form with type picker (word_filter/link_filter/spam_detection/caps_filter), pattern input,
  action picker (delete/warn/mute/flag)
