...
- `uploads.ex` (437L) — Presigned URL generation via `generate_presigned_url/2`, file validation,
  tier limits
- `voice_message.ex` (347L) — Full schema with waveform (float array), transcription, processing
  pipeline
- `ai/moderation.ex` (135L) — LLM + heuristic fallback, 7 categories, check/2 returns
  :allow/:flag/:block
- `feature_flags.ex` (368L) — GenServer with boolean/percentage/variant/targeted types, CRUD
  functions
- `rate_limit_plug.ex` (284L) — 9 scope presets, 4 algorithms (fixed_window, sliding_window,
...
**New files created:** ~35 **Existing files modified:** ~30 **New DB tables:**
moderation_audit_logs, (scheduled_messages migration if not exists) **New DB columns:** None
anticipated (schemas already exist) **New packages/deps:** None (all libraries already in deps)
