...
**Complete client-side multi-device E2EE key sync with cross-signing, encrypted key relay, and
...
- Cross-signing flow: new device registers → existing device prompted → user confirms → keys signed
  → material synced
...
- `apps/web/src/lib/crypto/e2ee-store/device-sync.ts` — Web device sync protocol (10 functions,
  15.9KB)
- `apps/mobile/src/lib/crypto/store/deviceSync.ts` — Mobile device sync (10 functions, 16.7KB)
- `apps/web/src/modules/chat/components/device-verification-dialog.tsx` — Dialog +
  LinkedDevicesPanel + KeyChangeBanner
- `apps/mobile/src/screens/settings/device-management-screen.tsx` — Full device management screen +
  KeyChangeBanner
