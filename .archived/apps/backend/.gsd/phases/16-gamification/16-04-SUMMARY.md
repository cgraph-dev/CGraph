...
**Status:** COMPLETE **Commits:** 6 tasks, 6 commits
...
- `purchase_battle_pass` deducts coins, sets `has_battle_pass: true`, retroactively awards premium
  rewards
...
- **Commit:** `2759331c`
- `purchase_listing/2` runs in `Repo.transaction`: validate → spend_coins(buyer) →
  award_coins(seller) → transfer ownership → mark sold
...
- **Commit:** `a02f4fa0`
- Battle pass tiers: real-time progress bar, claim button for reached tiers, premium track (gold
  border)
