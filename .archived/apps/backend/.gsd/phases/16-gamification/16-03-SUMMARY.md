...
**Status:** COMPLETE **Commits:** 7 tasks, 7 commits
...
- **Commit:** `e848b6e5`
- `CGraph.Gamification.FeatureGates` defines 15 level-gated features from level 1 (XP, streaks,
  achievements) to level 25 (prestige)
- Functions: `feature_requirements/0`, `required_level/1`, `unlocked?/2`, `get_user_gates/1`,
  `newly_unlocked_features/2`
- Progressive hierarchy: quests@3, leaderboard@5, shop@8, cosmetics@10, titles@12,
  animated_borders@15, marketplace@15, battle_pass@18, events@20, trading@20, prestige@25
...
- Returns 403 with `{error: "level_required", required_level: N, current_level: M}` for locked
  endpoints
...
- **Commit:** `16589bab`
- Protected routes: quests (level 3), leaderboard (level 5), shop (level 8), cosmetics (level 10),
  marketplace (level 15)
- New endpoint: `GET /gamification/feature-gates` returns all features with unlocked/locked status
  per user
...
- **Commit:** `e7ba38a7`
- `<LevelGate feature="marketplace">` wrapper renders children when unlocked, locked overlay
  otherwise
...
- Shared types added to `packages/shared-types/src/gamification.ts`: `FeatureGateStatus`,
