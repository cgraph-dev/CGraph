...
**One-liner:** Web and mobile group creation, channel management, invite flows, and settings all
wired to backend API with param alignment fixes.
...
- Fixed `groupsService.ts` param mapping: `avatar_url`/`banner_url` → `icon_url`/`banner_url`
...
Ready for Plan 11-03 (Explore page + channel threads). Group CRUD, channels, and invites are now
wired E2E across all platforms.
