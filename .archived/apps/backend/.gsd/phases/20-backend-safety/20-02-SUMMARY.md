...
- Fixed 30 user-facing `inspect(reason)` calls across 14 controllers:
  - coin_shop, marketplace, subscription, moderation, feature_flag, call, voting_actions, user,
    voice_message, theme, web_push, e2ee, creator, sync
- Added `alias CGraphWeb.ErrorHelpers` to each
- Logger-only `inspect()` calls preserved for debugging (stripe_webhook, auth, upload, gif, oauth,
  health controllers)
- Commit: `fc3dd838`
...
- `coin_checkout.ex:143` — now `Repo.get(User, purchase.user_id)` → `{:error, :user_not_found}` +
  log
- Commit: `2f6fa3e8`
...
- Bundle definitions now use `:stripe_env_key` atoms, resolved at runtime via
...
- **Tests not written**: Tests for controller error responses and CoinBundles config validation
  deferred per security-first priority. Structural changes are straightforward (string replacements,
  nil checks, config reads).
- **IAP controller inspect already fixed**: Was addressed in Plan 20-01 during JWS/RTDN verification
  work. The 3 remaining inspect calls in iap_controller.ex are Logger-only.
- **Earnings subquery approach**: Used subquery in SELECT instead of a CTE or single flat query.
  Ecto subquery in select is the most readable approach and executes as a single statement.
